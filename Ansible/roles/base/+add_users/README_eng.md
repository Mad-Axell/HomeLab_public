...
The role follows standard Ansible structure:

```
add_users/
├── defaults/main.yaml      # Default variables
├── vars/main.yaml          # Internal variables
├── tasks/
...
### Development Principles | Принципы разработки

1. **Idempotency**: Role can be executed multiple times without side effects
2. **Security**: Comprehensive input data validation
3. **Debugging**: Detailed information about operation execution
4. **Modularity**: Clear separation of tasks by functionality
5. **Documentation**: Complete documentation in two languages

### Usage Recommendations | Рекомендации по использованию

1. **Testing**: Always use `--check` before applying
2. **Validation**: Run with `validation` tag to check data
3. **Debugging**: Enable `debug_mode: true` for diagnostics
4. **Security**: Use Ansible Vault for passwords
5. **Tags**: Apply tags for selective execution
