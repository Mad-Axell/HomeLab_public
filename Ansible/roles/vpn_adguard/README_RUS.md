...
## Особенности туннеля, влияющие на вышестоящий роутер

Проверено на живой системе 2026-09-06 (AdGuard VPN CLI v1.7.12).

**ICMP через туннель не проходит.** Счётчики nftables на шлюзе: исходящих проб
28, ответов 0; TCP к тем же адресам при этом работает. Практическое следствие —
проверка состояния шлюза на вышестоящем роутере по ICMP **никогда** не пройдёт.
В pfSense у `dpinger` других методов нет, поэтому шлюзу нужен
`Disable Gateway Monitoring`, иначе он вечно `Offline`; а при
`Skip rules when gateway is down` роутер снимает правило policy routing целиком,
и вывод трафика перестаёт работать при полностью исправном туннеле. Отказ при
этом остаётся закрытым: если туннель упадёт, у контейнера исчезнет маршрут через
`tun0` и пакеты будут отброшены здесь.

**Туннель только IPv4.** `tun0` адреса IPv6 не получает. Если резолвер отдаёт
клиентам AAAA-записи, они пытаются IPv6, висят до таймаута и лишь потом
откатываются на IPv4: замер дал 4–15 с на `connect` вместо 0.08 с. Лечится не
здесь, а на резолвере — в AdGuard Home это `disable_ipv6`. Симптом легко принять
за медленный туннель, хотя туннель ни при чём.
...
| `vpn_adguard_lan_subnets` | list of strings | yes при включённом шлюзе | `[]` | Исходные CIDR, чей трафик маскарадится в туннель. Включайте сюда и транзитную подсеть до роутера, а не только клиентские сегменты — см. defaults. |
