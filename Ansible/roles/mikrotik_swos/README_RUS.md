...
- Явно использует классический адаптер SwOS для CSS326, обходя ошибочное автоматическое определение SwOS Lite в `mikrotik-swos==1.3.2`.
...
- Коммутатор доступен контроллеру по HTTP в доверенной management-сети; закреплённая библиотека выполняет HTTP Digest с переданными именем пользователя и паролем. SSH-доступ к коммутатору не используется и не требуется.
...
В `--check` роль выполняет HTTP-чтение и валидацию, но не создаёт backup и не отправляет POST-запросы. HTTP-модуль всегда делегируется на Ansible-контроллер (`localhost`), поэтому роль не пытается подключаться к коммутатору по SSH. Из-за чувствительной HTTP-аутентификации задача всегда использует `no_log: true` и `diff: false`; наружу выводится только безопасный факт планируемого изменения.
...
- Для CSS326 role-local module фиксирует классическую платформу SwOS через адаптер и cache API закреплённой версии библиотеки; смена версии зависимости требует повторной проверки этого контракта.
